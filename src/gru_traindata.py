if __name__ == "__main__":
    ...
    for i in range(xNum * yNum * zNum):
        ...
        concatp[3,:]=concatp[3,:]+shake[:15]
        concats[3, :] = concats[3, :] + shake[-15:]
        ...
        for h in range(wmnum):
            index = wmindex[h]
            if wmno[h] == 0:  # wrongp
                wrongpt = np.random.randint(0, 1000, size=1)[0] / 100
                while abs(wrongpt -concatpcopy[3,index]) < 0.1:
                    wrongpt = np.random.randint(0,1000, size=1)[0] / 100
            ...
            if wmno[h] == 1:  # wrongs
                wrongst = np.random.randint(0,  1000, size=1)[0] / 100
                while abs(wrongst -concatscopy[3,index]) < 0.1:
                    wrongst = np.random.randint(0,  1000, size=1)[0] / 100
        ...
        numcon=concatps.shape[0]
        cutnum=numcon-20
        cutno = np.random.choice(numcon-1, size=cutnum,replace=False)
        cutno = abs(np.sort(-cutno))
        if i%10000==0:
            print(concatps.shape,cutno,'1')
        for ii in range(cutnum):
            concatps=np.delete(concatps,cutno[ii],0)
